...
$tree = $("div.bbn-task-usertree", ele).kendoTreeView({
  ...
  template: function (e) {
    return '<i class="nf nf-fa-' + (e.item.is_parent ? 'users' : 'user') + '"> </i> ' + e.item.text;
  }
}).data("kendoTreeView");
    ...
    $.each(items, function(i, v){
      ...
      if ( $ele.is(":visible") ){
        ...
        $ul.find("li.k-item:last span").append(
          '&nbsp;',
          $('<i class="nf nf-fa-times bbn-p"/>').data("id", id).data("ele", $ele).click(function(){
            var $$ = $(this);
            $$.data("ele").show();
            v = $input.val();
            vals = v ? JSON.parse(v) : [];
      ...
      uploader.kendoUpload({
        ...
        template: function(e){
          ...
          if ( e.files && e.files[0] ){
            ...
            if ( !done ){
              st += '<i class="nf nf-fa-' + iconClass(e.files[0]) + '"> </i>';
            }
